...
let content = fs.readFileSync('E:/BandProject/OurBand/front/ourband/components/jam/AudioJamModal.tsx', 'utf-8');

// 1. Update Imports
const imports_target = `import { addHistoryCommentApi, increaseHistoryShareApi, toggleHistoryLikeApi } from "@/api/account/userService";
...
const imports_replacement = `import { addHistoryCommentApi, updateHistoryCommentApi, deleteHistoryCommentApi, increaseHistoryShareApi, toggleHistoryLikeApi, getUserInfoApi } from "@/api/account/userService";
import { apiClient } from "@/api/baseApi";
import { useUserProfile } from "@/store/userProfileContext";
...
// 2. Add States
const state_target = `  const [activeShareId, setActiveShareId] = useState<string | null>(null);
  
  const [commentText, setCommentText] = useState("");
  // 💡 히스토리인 경우 더미 댓글 배열을 비워둠으로써 실시간 카운트 동기화 방해를 막습니다.
  const [comments, setComments] = useState<any[]>([]);`;
const state_replacement = `  const [activeShareId, setActiveShareId] = useState<string | null>(null);
...
const get_comments_target = `        if (isJam) {
...
const get_comments_replacement = `        if (isJam) {
...
const get_history_replacement = `        } else {
          apiClient.get(\`/users/history/\${post.id}/comments\`)
            .then(res => {
              const mapComment = (c: any): any => ({
                id: c.id,
...
const handle_add_target = `  const handleAddComment = async () => {
...
const handle_add_replacement = `  const handleDeleteComment = async (commentId: number) => {
...
const render_comments_target = `                  <div className="flex-1 overflow-y-auto p-5 space-y-5 hide-scrollbar">
                    {comments.map((c, idx) => (
...
const render_comments_replacement = `                  <div className="flex-1 overflow-y-auto p-5 space-y-5 hide-scrollbar">
                    {(() => {
...
const input_target = `                  <div className="p-4 bg-background border-t border-border shrink-0">
...
const input_replacement = `                  <div className="p-4 bg-background border-t border-border shrink-0">
...
const close_target = `                <motion.div key="jam-modal-comment" 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} 
                className="absolute inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm"
                onClick={() => setActiveCommentId(null)}
              >`;
const close_replacement = `                <motion.div key="jam-modal-comment" 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} 
                className="absolute inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm"
...
const close_btn_target = `<button onClick={() => setActiveCommentId(null)} className="text-slate-400 hover:text-white"><X size={20}/></button>`;
const close_btn_replacement = `<button onClick={() => { setActiveCommentId(null); setReplyingTo(null); setEditingComment(null); setCommentText(''); }} className="text-slate-400 hover:text-white"><X size={20}/></button>`;
content = content.replace(close_btn_target, close_btn_replacement);

fs.writeFileSync('E:/BandProject/OurBand/front/ourband/components/jam/AudioJamModal.tsx', content, 'utf-8');
console.log('done');
