...
export default function CommunityPostDetailPage() {
    ...
    return (
      ...
      >
        <div
          ...
          onClick={() => openUserProfile(Number(c.userId), c.authorName, c.authorProfileImageUrl)}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-0.5">
              ...
              {!isAuthor && (
                <button
                  onClick={() => setReportTarget({ type: 'COMMUNITY_COMMENT', id: c.id, name: '댓글' })}
  ...
  if (errorMsg) {
    return <div className="min-h-screen bg-background text-white flex flex-col items-center justify-center">
      ...
      <button onClick={() => router.push('/community/free')} className="mt-6 px-6 py-2 bg-primary text-white rounded-lg hover:bg-indigo-600 transition-colors">목록으로 돌아가기</button>
    </div>;
  }
      ...
      <header className="px-6 py-4 bg-background/80 backdrop-blur-xl sticky top-0 z-20 border-b border-border flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {currentUserId !== post.userId && (
            <button onClick={() => setReportTarget({ type: 'COMMUNITY_POST', id: post.id, name: '게시물' })} className="text-slate-400 hover:text-rose-500 transition-colors p-2 rounded-full hover:bg-white/10"><AlertCircle size={20}/></button>
          )}
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto p-6 md:p-8 w-full text-left flex-1">
        ...
        <div className="flex justify-between items-center pb-6 border-b border-border/50 mb-8">
          <div 
            ...
            onClick={() => openUserProfile(Number(post.userId || post.author), post.author, post.authorProfileImageUrl)}
          >
