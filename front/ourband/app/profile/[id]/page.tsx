...
import { addFavoriteMusicApi, addGearApi, addHistoryApi, deleteFavoriteMusicApi, deleteGearApi, deleteHistoryApi, getUserInfoApi, getUserProfileApi, logoutApi, updateProfileApi, updateProfileImageApi, getUserFollowersApi, getUserFollowingsApi, toggleFollowApi, type FollowUser } from "@/api/account/userService";
...
export default function UserPublicProfilePage() {
  ...
  const handleOpenFollowModal = async (type: "follower" | "following") => {
    ...
    try {
      if (type === "follower") {
        const data = await getUserFollowersApi(targetUserId);
        setFollowerList(data);
      } else {
        const data = await getUserFollowingsApi(targetUserId);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await getUserProfileApi(targetUserId)
          .then(data => {
            setProfileData(data);
            setBio(data.bio || "");
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      {/* Cover Profile */}
      <div 
        className="relative h-64 md:h-80 bg-secondary border-b border-border overflow-hidden shadow-2xl relative group "
      ...
      <div className="px-6 md:px-10 lg:px-16 -mt-20 relative z-10 max-w-6xl mx-auto w-full">
        <div className="flex justify-between items-end mb-4">
          <div 
            className="w-28 h-28 md:w-36 md:h-36 rounded-full border-4 border-background bg-slate-800 overflow-hidden shadow-2xl relative group "
          ...
          <div className="lg:col-span-1">
            ...
            <section className="mb-10">
              ...
              <div className="space-y-3">
                ...
                <div key={band.bandId} className="flex items-center gap-4 p-4 rounded-2xl bg-secondary border border-border hover:border-primary/50 transition-colors  group">
            ...
            <section className="mb-10 bg-secondary/50 border border-border rounded-2xl p-5 grid grid-cols-2 gap-4 items-center text-center">
              ...
              <div className="border-x border-border  hover:bg-white/5 transition-colors py-2 rounded-lg" onClick={() => handleOpenFollowModal("follower")}>
              ...
              <div className="border-x border-border  hover:bg-white/5 transition-colors py-2 rounded-lg" onClick={() => handleOpenFollowModal("following")}>
                  ...
                  {profileData.histories.map((history, idx) => {
                    ...
                    return (
                      <div 
                        ...
                        className="bg-secondary/40 border border-border rounded-2xl overflow-hidden hover:border-primary/50 transition-colors  group flex flex-col relative mt-2" 
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {myJams.map((jam, i) => (
                      <div 
                        ...
                        className="group relative bg-background rounded-xl overflow-hidden border border-border  hover:border-primary transition-all aspect-[9/16]"
                      >
                 ...
                 {profileData.gears.length === 0 ? (
                   ...
                   profileData.gears.map((gear, idx) => (
                     <div key={`gear-${idx}`} className="flex items-center justify-between p-4 border-b border-border last:border-0 group transition-colors rounded-xl hover:bg-white/5">
                       <div className="flex items-center gap-4">
                         <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center shrink-0 border border-border">
                           <Settings2 className="text-slate-500" size={24} />
                         </div>
                         <div>
                           <div className="font-bold text-white text-base mb-1">{gear.gearName}</div>
                         </div>
                       </div>
                     </div>
                   ))
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <select
                        ...
                        className="w-full bg-secondary border border-border rounded-xl py-3.5 pl-4 pr-8 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white appearance-none "
                        required
                      >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select 
                      ...
                      className="w-full bg-secondary border border-border rounded-xl py-3.5 pl-12 pr-4 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white appearance-none "
                      required
                    >
          ...
          <motion.div key="history-modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-secondary w-full max-w-lg rounded-[2rem] p-6 border border-border shadow-2xl relative">
              ...
              <div className="space-y-4">
                ...
                <div
                  className={cn(
                    "border border-dashed rounded-xl flex flex-col items-center justify-center  transition-colors relative overflow-hidden h-100",
